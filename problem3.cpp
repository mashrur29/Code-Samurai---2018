...
void print_path(vector<pair<pair<string, string>, pair<int, int> > > tmp, string lat1, string lon1, string lat2, string lon2, int case_src, int case_dst) {
	...
	for (int i = 0; i < tmp.size() - 1; i++) {
		...
		if (type == 1) {
			if (metro_name.find(make_pair(tmp[i].first.first, tmp[i].first.second)) != metro_name.end())
				start_name = metro_name[make_pair(tmp[i].first.first, tmp[i].first.second)];
			if (metro_name.find(make_pair(tmp[i + 1].first.first, tmp[i + 1].first.second)) != metro_name.end())
			...
			if (uttara_name.find(make_pair(tmp[i].first.first, tmp[i].first.second)) != uttara_name.end())
				start_name = uttara_name[make_pair(tmp[i].first.first, tmp[i].first.second)];
			if (uttara_name.find(make_pair(tmp[i + 1].first.first, tmp[i + 1].first.second)) != uttara_name.end())
		...
		if (type == 0) {
			...
			string str1 = start_name;
			string str2 = endd_name;
			cout << "Ride Metro from " << " (" << tmp[i].first.first << ", " << tmp[i].first.second << ") ";
			cout << " to " << " (" << tmp[i].first.first << ", " << tmp[i].first.second << ") " << endl;
		}
		else if (type == 2) {
			...
			string str1 = start_name;
			string str2 = endd_name;
			cout << "Ride Bikolpo Bus from " << " (" << tmp[i].first.first << ", " << tmp[i].first.second << ") ";
			cout << " to " << " (" << tmp[i + 1].first.first << ", " << tmp[i + 1].first.second << ") " << endl;
		}
		else {
			cost = cst[make_pair(3, make_pair(start_id, dst_id))] * 7.0;
			//cout<<start_name<<","<<endd_name<<":"<<endl;
			cout << "Cost: ৳" << cost << ": ";
			//swap(start_name, endd_name);
			string str1 = start_name;
			string str2 = endd_name;
			cout << "Ride Uttara Bus from " << " (" << tmp[i].first.first << ", " << tmp[i].first.second << ") ";
			cout << " to " << " (" << tmp[i + 1].first.first << ", " << tmp[i + 1].first.second << ") ";
			cout << endl;
		}
...
int main() {
	...
	while (file3 >> row3) {
		...
		//cout<<row3[n_row3 - 2]<< " "<<row3[n_row3 - 1]<<endl;
