...
void print_path(vector<pair<pair<pair<string, string>, pair<int, int> >, ll > > tmp, string lat1, string lon1, string lat2, string lon2, int case_src, int case_dst) {
	...
	cout<< "Starting time at source: ";
	print_time(starting_time);
	cout<<endl;
	cout<< "Destination reaching time: ";
	print_time(ending_time);
	cout<<endl;
	ll cur_time;

	if(case_src == 3) {
		cur_time = tmp[0].second;
		print_time(starting_time);
		cout<<" - ";
		print_time(cur_time);
		cout<<", Cost: ৳0.00: Walk from Source (" << lat1 << ", " << lon1 << ") to (";
		cout<< tmp[0].first.first.first << ", " << tmp[0].first.first.second << ")" << endl;
	}
	if(case_src == 2) {
		cur_time = tmp[0].second;
		print_time(starting_time);
		cout<<" - ";
		print_time(cur_time);
		double diss = getDistanceFromLatLonInKm(stringToDouble(lat1), stringToDouble(lon1), stringToDouble(tmp[0].first.first.first), stringToDouble(tmp[0].first.first.second));
		diss = diss*20.0;
		cout<<", Cost: ৳"<<diss<<": Ride Car from Source (" << lat1 << ", " << lon1 << ") to (";
		cout<< tmp[0].first.first.first << ", " << tmp[0].first.first.second << ")" << endl;
	}

	ll prev_time = starting_time;
	...
	for (int i = 0; i < tmp.size() - 1; i++) {
		...
		int start_id = compress[make_pair(tmp[i].first.first.first, tmp[i].first.first.second)];
		int dst_id = compress[make_pair(tmp[i+1].first.first.first, tmp[i+1].first.first.second)];

		if(i>0) prev_time = tmp[i].second;

		cur_time = tmp[i+1].second;
		...
		if(type == 0) {
			...
			cout<<"Ride Car from "<<str1<<" ("<<tmp[i].first.first.first<<", "<< tmp[i].first.first.second<<") ";
			cout<<" to "<<str2<<" ("<<tmp[i+1].first.first.first<<", "<< tmp[i+1].first.first.second<<") "<<endl;
		}
		else if(type == 1) {
			...
			string str1=start_name;
			string str2=endd_name;
			cout<<"Ride Metro from "<<str1<<" ("<<tmp[i].first.first.first<<", "<< tmp[i].first.first.second<<") ";
			cout<<" to "<<str2<<" ("<<tmp[i+1].first.first.first<<", "<< tmp[i+1].first.first.second<<") "<<endl;	
		}
		else if(type == 2) {
			...
			string str1=start_name;
			string str2=endd_name;
			cout<<"Ride Bikolpo Bus from "<<str1<<" ("<<tmp[i].first.first.first<<", "<< tmp[i].first.first.second<<") ";
			cout<<" to "<<str2<<" ("<<tmp[i+1].first.first.first<<", "<< tmp[i+1].first.first.second<<") "<<endl;	
		}
		else {
			...
			string str1=start_name;
			string str2=endd_name;
			cout<<"Ride Uttara Bus from "<<str1<<" ("<<tmp[i].first.first.first<<", "<< tmp[i].first.first.second<<") ";
			cout<<" to "<<str2<<" ("<<tmp[i+1].first.first.first<<", "<< tmp[i+1].first.first.second<<") "<<endl;	
		}
	...
	if(case_dst == 3) {
		prev_time = tmp[n_t-1].second;
		cur_time = tmp[n_t-1].second + 60;
		print_time(prev_time);
		cout<<" - ";
		print_time(cur_time);
		cout<<", Cost: ৳0.00: Walk from (" << tmp[n_t-1].first.first.first << ", " << tmp[n_t-1].first.first.second << ") to Destination (";
		cout<< lat2 << ", " << lon2 << ")" << endl;
	}
	else if(case_dst == 2) {
		prev_time = tmp[n_t-1].second;
		...
		cout<<" - ";
		print_time(cur_time);
		double diss = getDistanceFromLatLonInKm(stringToDouble(tmp[0].first.first.first), stringToDouble(tmp[0].first.first.second), stringToDouble(lat2), stringToDouble(lon2));
		diss = diss*20.0;
		cout<<", Cost: ৳"<<diss<<": Ride Car from (" << tmp[n_t-1].first.first.first << ", " << tmp[n_t-1].first.first.second << ") to Destination (";
		cout<< lat2 << ", " << lon2 << ")" << endl;
	}
}
...
void test() {
	//90.439412,23.737918, 90.444317,23.691768
	string lat1 = "90.439412";
	string lon1 = "22.737918";
	...
	string lat2 = "90.444317";
	string lon2 = "23.691768";
...
int main() {
	...
	ifstream file1("/home/mashrur/Desktop/BJIT/Code Samurai 2019 - Problem Resources/Routemap-DhakaMetroRail.csv");
	ifstream file2("/home/mashrur/Desktop/BJIT/Code Samurai 2019 - Problem Resources/Routemap-BikolpoBus.csv");
	ifstream file3("/home/mashrur/Desktop/BJIT/Code Samurai 2019 - Problem Resources/Routemap-UttaraBus.csv");
	...
	while (file1 >> row1) {
		...
		//w = w * 5.0;
		cst[make_pair(1, make_pair(u, v))] = w;
		cst[make_pair(1, make_pair(v, u))] = w;
	...
	while (file2 >> row2) {
		...
		cst[make_pair(2, make_pair(u, v))] = w;
		cst[make_pair(2, make_pair(v, u))] = w;
	...
	while (file3 >> row3) {
		...
		cst[make_pair(3, make_pair(u, v))] = w;
		cst[make_pair(3, make_pair(v, u))] = w;
